...
async function createTestUsers() {
    ...
    const worker1 = await userDao.create({ name: 'Worker One',   role: 2, email: 'worker1@test.com', password: 'pass' });
    const worker2 = await userDao.create({ name: 'Worker Two',   role: 2, email: 'worker2@test.com', password: 'pass' });
...
test('Create new project', async function () {
    ...
    const created = await projectDao.create({
        name: 'Project', manager: manager._id, location: 'Baltimore',
...
test('Read all projects', async function () {
    ...
    await projectDao.create({ name: 'Project1', manager: manager._id, location: 'Baltimore', workers: [] });
    await projectDao.create({ name: 'Project2', manager: manager._id, location: 'New Jersey', workers: [] });
...
test('Update project', async function () {
    const { manager } = await createTestUsers();
    const created = await projectDao.create({ name: 'Old Name', manager: manager._id, location: 'New Jersey', workers: [] });
    const updated = await projectDao.update(created._id, { name: 'Updated Name' });
    expect(updated.name).toBe('Updated Name');
});

test('Delete project', async function () {
    const { manager } = await createTestUsers();
    const created = await projectDao.create({ name: 'Test Delete', manager: manager._id, location: 'Baltimore', workers: [] });
...
test('Delete all projects', async function () {
    const { manager } = await createTestUsers();
    await projectDao.create({ name: 'Project1', manager: manager._id, location: 'Baltimore', workers: [] });
    await projectDao.create({ name: 'Project2', manager: manager._id, location: 'CA', workers: [] });
    await projectDao.deleteAll();
    const projects = await projectDao.readAll();
